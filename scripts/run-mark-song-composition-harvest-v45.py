#!/usr/bin/env python3
"""Execute Mark V45 Phase A without opening Song semantics.

This runner intentionally reuses the frozen V9 anonymous operator engine as a
narrow measurement carrier. It adds only V45 custody, a topology-only
...
REF = ROOT / "scripts" / "mark-v45"
PRIMARY_PROTOCOL = ROOT / "research" / "mark" / "discovery-experiments" / "song-composition-harvest-v45.operator-reference.v9.json"
TOPOLOGY_PROTOCOL = ROOT / "research" / "mark" / "discovery-experiments" / "song-composition-harvest-v45.operator-reference-topology.v9.json"
...
def main():
    ...
    if not config.get("implementation_sha") or config.get("preexecution_freeze_sha") in (None,"","PENDING"):
        raise RuntimeError("V45 refuses to execute before implementation and preexecution freeze SHAs are recorded")
    if git_blob_sha1(__file__)!=config["runner_blob_sha1"]:
        raise RuntimeError("V45 runner bytes differ from frozen config")
    for path,key in ((PRIMARY_PROTOCOL,"primary_protocol_blob_sha1"),(TOPOLOGY_PROTOCOL,"topology_protocol_blob_sha1")):
        if git_blob_sha1(path)!=config[key]:
            raise RuntimeError("V45 operator reference protocol drift")
    engine_paths={
    ...
    for key,path in engine_paths.items():
        if git_blob_sha1(path)!=config["reference_engine_blobs"][key]:
            raise RuntimeError(f"V45 engine blob drift: {key}")
    eq=REF/"test-mark-v45-implementation-equivalence.py"
    if git_blob_sha1(eq)!=config["implementation_equivalence_blob_sha1"]:
        raise RuntimeError("V45 implementation-equivalence test blob drift")
    ...
    if manifest["eligibleObservations"]!=config["inputs"]["pair_eligible_observations"]:
        raise RuntimeError("V45 split observation count drift")
    for lane,expected in config["inputs"]["lane_inventory"].items():
        if manifest["lanes"][lane]["observations"]!=expected["observations"]:
            raise RuntimeError("V45 lane inventory drift")
    train_eq=REF/"compare-mark-v44-v45-train-freeze.py"
    if git_blob_sha1(train_eq)!=config["train_freeze_equivalence_blob_sha1"]:
        raise RuntimeError("V45 train-freeze equivalence test blob drift")
    run_cmd(train_eq,{
        "MARK_V45_SPLIT":str(split_dir),
        "MARK_V45_PRIMARY_PROTOCOL":str(PRIMARY_PROTOCOL),
        "MARK_V45_TOPOLOGY_PROTOCOL":str(TOPOLOGY_PROTOCOL),
        "MARK_V45_EQ_WORK":str(work/"train-equivalence"),
    })
    ...
    for spec in config["candidate_specs"]:
        ...
        if law=="composition":
            ...
            failures=[] if status=="HARVESTED_RULE" else ["factorized composition did not clear every frozen V45 transfer/invariance gate"]
        else:
    ...
    packet={
        "schema":"mark_song_composition_harvest_v45_phase_a_candidates_v1",
        "experiment_id":"mark:song-composition-harvest:v45",
        "phase":"A",
