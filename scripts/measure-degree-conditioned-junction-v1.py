#!/usr/bin/env python3
"""Extract source-local degree-conditioned junction law states."""
...
null_n=int(protocol["measurement"]["null_iterations"])
min_context=30
...
pat=re.compile(r"^CENTER:JUNCTION\|DEGREE:(\d+)\|ARM:(PATH_TO_(?:ENDPOINT|JUNCTION))$")

contexts=[r[0] for r in con.execute(
    "select distinct context from grammar_stats where iteration=-1 and lane='train' and context like 'CENTER:JUNCTION|DEGREE:%|ARM:PATH_TO_%' order by context"
)]
...
print(json.dumps({
  "schema":"degree_conditioned_junction_source_metrics_v1",
  "sourceGroupId":args.source_id,
  "originalBlindLane":args.lane,
