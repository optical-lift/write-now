...
def operator_occurrence(graph, incoming_neighbor, center, outgoing_neighbor, cfg):
    # V45 memory amendment: compute the exact frozen transit representation
    # on demand instead of retaining every ordered transit for the life of
    # a large observation. The descriptor/IDs/states are unchanged.
...
def build_probability_functions(model_variant):
    ...
    def _pcomp_cached(a, b, i, o):
        return sum(pop(a, i, middle) * pop(b, middle, o) for middle in states)

    def pcomp(a, b, i, o):
        if i not in state_set: i = "OTHER"
        if o not in state_set: o = "OTHER"
        return _pcomp_cached(a, b, i, o)

    # Expose only cache diagnostics; this does not participate in probability calculation.
    pcomp.cache_info = _pcomp_cached.cache_info
    pcomp.cache_clear = _pcomp_cached.cache_clear
