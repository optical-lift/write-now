#!/usr/bin/env python3
"""No-refit confirmation for the seven frozen V47 pairwise transition rules."""
...
GEOMETRY_SHA="812aba2f2e813f3d53961753e48d1ee510cad937817815c7dee36ba1f9cc2ad5"
...
def load_geometry(path:Path):
    if sha(path)!=GEOMETRY_SHA: raise RuntimeError("confirmation geometry hash drift")
    rows=[]; by_id={}
    with path.open() as f:
        for line in f:
            r=json.loads(line)
    ...
    if len(rows)!=EXPECTED_ROWS or len(set(r["sourceGroupId"] for r in rows))!=EXPECTED_SOURCES: raise RuntimeError("confirmation geometry inventory drift")
    return rows,by_id
...
def main():
    ...
    for r in rows:
        ...
        if pid not in by_id: raise RuntimeError("parent outside confirmation geometry")
        edges.append({
    ...
    if len(edges)!=EXPECTED_EDGES: raise RuntimeError("confirmation edge count drift")
    ...
    for d in candidates:
        ...
        if insufficient:
            ...
            status="TRANSFERRED" if all(gates.values()) else "REJECTED_ON_CONFIRMATION"

        adjudicated.append({
            "transitionKey":key,
            "status":status,
            "confirmationContextOccurrences":total,
            "confirmationTargetOccurrences":hit_n,
            "confirmationTargetDistinctSources":hit_sources,
            "confirmationConditionalProbability":cond,
            "confirmationScalePairParentMarginal":base,
            "confirmationLift":lift,
    ...
    rejected=sum(r["status"]=="REJECTED_ON_CONFIRMATION" for r in adjudicated)
    insufficient=sum(r["status"]=="INSUFFICIENT_CONTRAST" for r in adjudicated)
    supported=(rejected==0 and transferred>=5)

    out={
        "schema":"mark_regime_transition_pairwise_confirmation_v47_v1",
        ...
        "status":"CONFIRMATION_SUPPORTED" if supported else "CONFIRMATION_RESIDUAL",
        "confirmationSupported":supported,
        "refitPerformed":False,
        "provenanceOpened":False,
        "provenanceConsumed":False,
    }
    args.out.mkdir(parents=True,exist_ok=True)
    (args.out/"pairwise-confirmation.json").write_text(json.dumps(out,indent=2)+"\n")
    print(json.dumps({
        "status":out["status"],
