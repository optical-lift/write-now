...
function foregroundComponents(mask,w,h){
  return connectedSets(mask,w,h,1,N8).map((group,i)=>({id:`C${String(i+1).padStart(4,"0")}`,...summarizeSet(group.members,w),pixels:group.members}));
}
...
function traceSkeletonPaths(skeleton,w,h,criticalNodes){
  ...
  const visited=new Set(),edges=[];const pixelEdgeKey=(a,b)=>a<b?`${a}-${b}`:`${b}-${a}`;
  for(const node of criticalNodes)for(const p of node.pixels)for(const ni of neighborIndexes(p,w,h)){
    ...
    const firstKey=pixelEdgeKey(p,ni);if(visited.has(firstKey))continue;visited.add(firstKey);
    ...
    while(guard++<skeleton.length+5){
      ...
      const next=nexts.find(x=>!visited.has(pixelEdgeKey(cur,x)));if(next===undefined)break;
...
function holeNodes(mask,w,h,componentByPixel){
  ...
  for(let i=0;i<holes.length;i+=1){
    ...
    for(const p of group.members)for(const ni of neighborIndexes(p,w,h))if(mask[ni]){const cid=componentByPixel[ni];if(cid)touches.set(cid,(touches.get(cid)??0)+1);}
...
function isPureCycleComponent(componentId,skeleton,componentByPixel,w,h){
  const pixels=[];for(let i=0;i<skeleton.length;i+=1)if(skeleton[i]&&componentByPixel[i]===componentId)pixels.push(i);
  if(pixels.length<3)return false;
  return pixels.every(i=>neighborIndexes(i,w,h).reduce((n,ni)=>n+(skeleton[ni]&&componentByPixel[ni]===componentId?1:0),0)===2);
}
...
export function buildRelationalGraph(mask,width,height,{observationId=null,sourceGroupId=null}={}){
  const skeleton=thin(mask,width,height),components=foregroundComponents(mask,width,height),componentByPixel={};
  ...
  for(const node of critical){const votes=new Map();for(const p of node.pixels){const cid=componentByPixel[p];if(cid)votes.set(cid,(votes.get(cid)??0)+1);}criticalComponent.set(node.id,[...votes.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0]))[0]?.[0]??null);}
  ...
  const cycleComponents=new Set(components.filter(component=>isPureCycleComponent(component.id,skeleton,componentByPixel,width,height)).map(component=>component.id));
