...
function makeCriticalNodes(skeleton,w,h){
  ...
  const nodes=[];
  for(const [kind,mask] of [["ENDPOINT",endpointMask],["JUNCTION",junctionMask]]){
    for(const group of connectedSets(mask,w,h,1,N8))nodes.push({kind,pixels:group.members,...summarizeSet(group.members,w)});
  }
...
function skeletonComponents(skeleton,w,h){
  const sets=connectedSets(skeleton,w,h,1,N8);
  return sets.map((group,i)=>({id:`C${String(i+1).padStart(4,"0")}`,...summarizeSet(group.members,w),pixels:group.members}));
}
...
function traceSkeletonPaths(skeleton,w,h,criticalNodes){
  const nodeByPixel=new Map();
  for(const node of criticalNodes)for(const p of node.pixels)nodeByPixel.set(p,node.id);
  const visited=new Set(), edges=[];
  const pixelEdgeKey=(a,b)=>a<b?`${a}-${b}`:`${b}-${a}`;
  for(const node of criticalNodes){
    for(const p of node.pixels){
      for(const ni of neighborIndexes(p,w,h)){
        if(!skeleton[ni]||nodeByPixel.get(ni)===node.id)continue;
        const firstKey=pixelEdgeKey(p,ni); if(visited.has(firstKey))continue;
        visited.add(firstKey);
        let prev=p,cur=ni,length=1,target=null,guard=0;
        while(guard++<skeleton.length+5){
          const owning=nodeByPixel.get(cur);
          if(owning&&owning!==node.id){target=owning;break;}
          const nexts=neighborIndexes(cur,w,h).filter(x=>skeleton[x]&&x!==prev);
          if(!nexts.length)break;
          const next=nexts.find(x=>!visited.has(pixelEdgeKey(cur,x)));
          if(next===undefined)break;
          visited.add(pixelEdgeKey(cur,next)); prev=cur;cur=next;length+=1;
        }
        if(target)edges.push({source:node.id,target,relation:"PATH",pathLength:length});
      }
    }
  }
  return edges;
...
function holeNodes(mask,w,h,componentByPixel){
  ...
  for(let i=0;i<holes.length;i+=1){
    ...
    for(const p of group.members)for(const ni of neighborIndexes(p,w,h))if(mask[ni]){
      const cid=nearestComponentId(ni,componentByPixel,w,h);if(cid)touches.set(cid,(touches.get(cid)??0)+1);
    }
...
function incidentDescriptors(graph,nodeId,labels){
  ...
  for(const edge of graph.edges){
    if(SYMMETRIC_RELATIONS.has(edge.relation)){
      if(edge.source===nodeId)out.push(`UND:${edge.relation}:${labels.get(edge.target)}`);
      else if(edge.target===nodeId)out.push(`UND:${edge.relation}:${labels.get(edge.source)}`);
      continue;
    }
...
export function canonicalRelationalFingerprint(graph,{iterations=3}={}){
  ...
  for(let round=0;round<iterations;round+=1){
    ...
    for(const node of graph.nodes){
      const neighborhood=incidentDescriptors(graph,node.id,labels).join("|");
      next.set(node.id,hash(`${labels.get(node.id)}||${neighborhood}`).slice(0,24));
    }
    labels=next;
  }
  const nodeMultiset=[...labels.values()].sort();
  const edgeMultiset=graph.edges.map(edge=>{
    const a=labels.get(edge.source),b=labels.get(edge.target);
    if(SYMMETRIC_RELATIONS.has(edge.relation)){const pair=[a,b].sort();return `${pair[0]}:${edge.relation}:${pair[1]}`;}
    return `${a}:${edge.relation}:${b}`;
  }).sort();
...
export function localRelationalMotifs(graph,{radius=1}={}){
  ...
  for(let round=0;round<radius;round+=1){
    ...
    for(const node of graph.nodes){
      const neighborhood=incidentDescriptors(graph,node.id,labels).join("|");
      next.set(node.id,`${graphNodeLabel(node)}{${neighborhood}}`);
    }
    labels=next;
  }
...
export function relationGrammarPaths(graph){
  ...
  for(const edge of graph.edges){
    if(SYMMETRIC_RELATIONS.has(edge.relation)){
      incident.get(edge.source)?.push({neighbor:edge.target,token:`UND:${edge.relation}`});
      incident.get(edge.target)?.push({neighbor:edge.source,token:`UND:${edge.relation}`});
    }else{
      incident.get(edge.source)?.push({neighbor:edge.target,token:`OUT:${edge.relation}`});
      incident.get(edge.target)?.push({neighbor:edge.source,token:`IN:${edge.relation}`});
    }
  }
  ...
  for(const center of graph.nodes){
    ...
    for(let i=0;i<arms.length;i+=1)for(let j=i+1;j<arms.length;j+=1){
      ...
      const left=`${nodeKind.get(a.neighbor)}|${a.token}`,right=`${b.token}|${nodeKind.get(b.neighbor)}`;
...
export function buildRelationalGraph(mask,width,height,{observationId=null,sourceGroupId=null}={}){
  const skeleton=thin(mask,width,height),components=skeletonComponents(skeleton,width,height);
  const componentByPixel={};
  for(const component of components)for(const p of component.pixels)componentByPixel[p]=component.id;
  const critical=makeCriticalNodes(skeleton,width,height);
  const criticalComponent=new Map();
  ...
  const holes=holeNodes(mask,width,height,componentByPixel);
  const nodes=[];
  ...
  let cycleIndex=0;
  for(const cid of [...cycleComponents].sort())nodes.push({id:`Y${String(++cycleIndex).padStart(4,"0")}`,kind:"CYCLE",componentId:cid});
  ...
  for(const pathEdge of traceSkeletonPaths(skeleton,width,height,critical))edges.push({source:pathEdge.source,target:pathEdge.target,relation:"PATH"});
  ...
  graph.fingerprint=canonicalRelationalFingerprint(graph);
  graph.motifs=localRelationalMotifs(graph,{radius:1});
  graph.grammarPaths=relationGrammarPaths(graph);
...
export function relationHistogram(graph){
  const nodes={},edges={};
  for(const node of graph.nodes)nodes[graphNodeLabel(node)]=(nodes[graphNodeLabel(node)]??0)+1;
...
export function histogramDelta(from,to){
  ...
  const nodes=Object.fromEntries(nodeKinds.map(k=>[k,(b.nodes[k]??0)-(a.nodes[k]??0)]).filter(([,v])=>v!==0));
  const edges=Object.fromEntries(edgeKinds.map(k=>[k,(b.edges[k]??0)-(a.edges[k]??0)]).filter(([,v])=>v!==0));
  return {nodes,edges};
}

export function transformationSignature(from,to){
  const delta=histogramDelta(from,to);
  return hash(JSON.stringify(delta)).slice(0,32);
}
