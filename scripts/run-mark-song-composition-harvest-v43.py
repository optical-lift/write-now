#!/usr/bin/env python3
"""Execute Mark V43 Phase A without opening Song semantics.

This runner intentionally reuses the frozen V9 anonymous operator engine as a
narrow measurement carrier. It adds only V43 custody, a topology-only
...
REF = ROOT / "scripts" / "mark-v43"
PRIMARY_PROTOCOL = ROOT / "research" / "mark" / "discovery-experiments" / "song-composition-harvest-v43.operator-reference.v9.json"
TOPOLOGY_PROTOCOL = ROOT / "research" / "mark" / "discovery-experiments" / "song-composition-harvest-v43.operator-reference-topology.v9.json"
...
def main():
    ...
    for spec in config["candidate_specs"]:
        ...
        if law=="composition":
            ...
            failures=[] if status=="HARVESTED_RULE" else ["factorized composition did not clear every frozen V43 transfer/invariance gate"]
        else:
    ...
    packet={
        "schema":"mark_song_composition_harvest_v43_phase_a_candidates_v1",
        "experiment_id":"mark:song-composition-harvest:v43",
        "phase":"A",
