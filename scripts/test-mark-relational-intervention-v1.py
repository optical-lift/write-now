        ...
        for j in range(i + 1, len(pool)):
            ...
            for alt_index, (new_a, new_b) in enumerate(alternatives):
                ...
                if new_a[0] == new_a[1] or new_b[0] == new_b[1]:
                    continue
                rewired = clone_and_rewire(base, pair_a, pair_b, new_a, new_b)
                if component_count(rewired) != original_sig["components"]:
                    continue
                key = hashlib.sha256(
                    f"{observation_id}|{pair_a}|{pair_b}|{new_a}|{new_b}|{alt_index}".encode()
                ).hexdigest()
                candidates.append((key, pair_a, pair_b, new_a, new_b, len_a, len_b, mult_a))
    if not candidates:
        return None
    _, pair_a, pair_b, new_a, new_b, len_a, len_b, mult = min(candidates)
