...
const nullA=evaluation.transferA?.rewiredNull?.diagnostics??{},nullB=evaluation.transferB?.rewiredNull?.diagnostics??{};
const checks=[
  ...
  ["evaluation_schema",evaluation.schema==="mark_relational_transfer_evaluation_blind_v2"],
  ["no_presumed_negative_category",evaluation.evaluationContract?.categoryAssumption===false],
  ["rewired_null_iterations_exercised",evaluation.transferA?.rewiredNull?.iterations>=4&&evaluation.transferB?.rewiredNull?.iterations>=4],
  ["transfer_a_null_actually_changed_graphs",nullA.totalAcceptedSwaps>0&&nullA.changedDistinctObservations>0&&nullA.changedGraphRuns>0],
  ["transfer_b_null_actually_changed_graphs",nullB.totalAcceptedSwaps>0&&nullB.changedDistinctObservations>0&&nullB.changedGraphRuns>0],
...
console.log(`NULL transfer_a accepted_swaps=${nullA.totalAcceptedSwaps??0} changed_distinct_observations=${nullA.changedDistinctObservations??0} changed_graph_run_rate=${nullA.changedGraphRunRate??0}`);
console.log(`NULL transfer_b accepted_swaps=${nullB.totalAcceptedSwaps??0} changed_distinct_observations=${nullB.changedDistinctObservations??0} changed_graph_run_rate=${nullB.changedGraphRunRate??0}`);
