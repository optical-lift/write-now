...
if(!["mark_blind_world_holdout_prediction_v1","mark_blind_world_holdout_prediction_v2"].includes(prediction.schema))throw new Error(`unsupported prediction ${prediction.schema}`);
const worldCore={...buildWorldModel(measurements.records),schema:"mark_blind_world_model_full_v1",generatedAt:new Date().toISOString(),sourceMeasurementsSha256:measurements.blindSha256,validationPredictionSha256:prediction.blindSha256,assemblyContract:{holdoutPredictionsSealedBeforeFullWorldAssembly:true,contextLabelsAvailable:false,forcedHoldoutAssignmentRequired:false},limitations:["The full world is assembled only after holdout evaluation is sealed.","This is a structural world graph, not an assertion that all recurrent structures share one historical origin."]};
